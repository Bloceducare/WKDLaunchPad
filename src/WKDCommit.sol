...
contract WKDCommit {
    ...
    function initialize(address _wkd) external  {
        // require(msg.sender == admin, "WKDCommit: FORBIDDEN");
        require(!isInitialized, "WKDCommit: already initialized");
        wkd = IBEP20(_wkd);
        isInitialized = true;
    ...
    function removeWkd(uint256 _amount) public {
        ...
        require(
            userCommit[msg.sender] >= _amount,
            "WKDCommit: Amount must be less than user's commit"
        );
     ...
     */
    function recoveryToken(address _tokenAddress, uint256 _tokenAmount)
        external
    {
        require(
            _tokenAddress != address(wkd),
            "WKDCommit: Cannot be WKD token"
        );
        require(msg.sender == admin, "WKDCommit: FORBIDDEN");
